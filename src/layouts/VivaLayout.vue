<template>
  <q-layout view="hHh lpR fff" class="bg-grey-1 vivareal">
    <q-header reveal elevated class="text-white" height-hint="98">
      <q-toolbar>
        <q-toolbar-title>
          <q-img class="q-ma-sm cursor-pointer" style="max-heigh: 30px; max-width: 120px;" src="img/logoVivaRealBr.svg" @click="voltaHome()"></q-img>
        </q-toolbar-title>
        <q-tabs align="left" class="text-white viva-tab">
          <q-route-tab to="/vivareal/venda" label="Venda" />
          <q-route-tab to="/vivareal/aluguel" label="Aluguel" />
        </q-tabs>
        <q-btn flat color="white" icon="arrow_back_ios_new" @click="voltarPag()" />
      </q-toolbar>
    </q-header>
...
export default {
  ...
  methods: {
    carregaInfo () {
      ...
      document.querySelector('title').innerText = 'Viva Real'
      const icons = ['128x128', '96x96', '32x32', '16x16']
      icons.forEach(obj => {
        document.querySelector(`link[sizes="${obj}"]`).setAttribute('href', `icons/vivareal/favicon-${obj}.png`)
      })
      document.querySelector('link[type="image/ico"]').setAttribute('href', 'icons/vivareal/favicon.ico')
    },
    voltarPag () {
      this.$router.go(-1)
    },
    voltaHome () {
      this.$router.push('/vivareal')
    },
    listaImoveis (d) {
      let dados = d
      dados.forEach(obj => {
      ...
      dados = dados.filter(obj => {
        ...
        const maxAluguel = obj.boundingBox ? this.valores.aluguel * 0.9 : this.valores.aluguel
        const maxVenda = obj.boundingBox ? this.valores.venda * 0.9 : this.valores.venda
        ...
        return (latitude && longitude && area && ativo) && ((aluguel && aluguelVlr <= maxAluguel) || (venda && vendaVlr <= maxVenda && metQrd > this.metQrdMin)) ? obj : ''
      })
